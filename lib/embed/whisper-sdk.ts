...
/* =========================
   ORIGINAL SDK CONTINUES (UNCHANGED)
========================= */
...
function boot(cfg){
  ...
  function getConsoleLogs(){
    // ALWAYS include logs if they exist (even if config disables capture).
    try{return typeof window.getConsoleLogs==='function'?window.getConsoleLogs():[];}catch(_){return [];}
  }
  ...
  (function(){
    try{
      if(typeof window.fetch==='function' && !window.__whisper_fetch_patched){
        window.__whisper_fetch_patched=true;
        var of=window.fetch;
    ...
    try{
      ...
      if(X&&X.prototype && !window.__whisper_xhr_patched){
        window.__whisper_xhr_patched=true;
  ...
  send.addEventListener('click',function(){
    ...
    try{console.log('Whisper sending logs:',logs);}catch(_){}
    // Only send failing network requests (4xx/5xx + request errors)
    var badNet=[];
    try{
      badNet=(net||[]).filter(function(r){
        if(!r)return false;
        if(r.ok===false)return true;
        var st=typeof r.status==='number'?r.status:null;
        return st!=null && st>=400;
      });
    }catch(_){badNet=[];}
    try{console.log('Whisper sending bad network:',badNet);}catch(_){}
    ...
    fetch(W+'/api/widget/feedback',{
      ...
      body:JSON.stringify({message:msg,context:ctx,events:sessionOn?events:[],console:logs,network:badNet,receiptAt:Date.now()})
    }).then(function(r){
